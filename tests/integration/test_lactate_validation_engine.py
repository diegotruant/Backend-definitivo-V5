...
print("\n[5] Module self-demo")
import subprocess
demo_script = Path(__file__).resolve().parents[2] / "engines" / "metabolic" / "lactate_validation_engine.py"
r = subprocess.run([sys.executable, str(demo_script)], capture_output=True, text=True, cwd=str(Path(__file__).resolve().parents[2]))
