"""Internal scientific gate for a longitudinal MMP curve.

This module is intentionally NOT wired to FastAPI, TwinState or Supabase yet.
It evaluates the canonical ``rolling_power_curve`` and returns an internal,
JSON-safe assessment that can later back ``mmp_state.v1`` after frontend
coordination.
...
def evaluate_mmp_gate(
    ...
    *,
    ...
    mmp = {duration_s: float(entry["power_w"]) for duration_s, entry in curve.items()}
    samples = [
        {
            "duration_s": duration_s,
    ...
    blocking_issues = [
        issue.to_dict() for issue in quality_report.issues if issue.severity == "error"
    ]
    ...
    critical_durations = sorted(
        ...
        if any(start_s <= duration_s <= end_s for start_s, end_s, _ in BAND_REQUIREMENTS.values())
    )
    ...
    non_missing_band_count = sum(value != "missing" for value in coverage.values())
    core_bands_available = (
        coverage["vo2"] != "missing" and coverage["threshold"] != "missing"
    )
    ...
    provisional = (
        ...
        and not any(
            duration_s in stale_durations or duration_s in missing_date_durations
            for duration_s in critical_durations
