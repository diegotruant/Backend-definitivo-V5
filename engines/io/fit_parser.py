...
def _read_file_with_retry(path: str, attempts: int = 3, delay_s: float = 0.25) -> bytes:
    ...
    if last_err is not None:
        raise last_err
    raise OSError(f"Unable to read {path!r} after {attempts} attempts")
...
class ActivityStreamEnhanced:
    ...
    def __init__(
        ...
        # Time and position
        self.elapsed_s: np.ndarray = np.zeros(n_samples, dtype=np.float32)
        self.lat: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.lon: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.altitude_m: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.distance_m: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)

        # Core metrics (canonical names — no compat aliases)
        self.power: np.ndarray = np.zeros(n_samples, dtype=np.float32)
        self.heart_rate: np.ndarray = np.zeros(n_samples, dtype=np.float32)
        self.cadence: np.ndarray = np.zeros(n_samples, dtype=np.float32)
        self.speed_mps: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.temperature_c: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        ...
        self.left_right_balance: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        ...
        self.left_power_phase: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.right_power_phase: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.left_power_phase_peak: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.right_power_phase_peak: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.left_pco: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.right_pco: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.left_pedal_smoothness: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.right_pedal_smoothness: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.left_torque_effectiveness: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.right_torque_effectiveness: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.respiration_rate: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        # Standing/seated position (0=seated, 1=standing) and dynamics flags.
        self.cadence_position: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        ...
        self.core_body_temp: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.skin_temp: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.ambient_temp: np.ndarray = np.full(n_samples, np.nan, dtype=np.float32)
        self.has_core_sensor: bool = False

        # Quality flags
        self.quality_power: np.ndarray = np.full(n_samples, QUALITY_GOOD, dtype=np.uint8)
        self.quality_hr: np.ndarray = np.full(n_samples, QUALITY_GOOD, dtype=np.uint8)
...
def detect_and_fill_gaps(
    ...
    while i < n:
        ...
        gap_duration_s: float = float(
            elapsed[gap_end - 1] - (elapsed[gap_start - 1] if gap_start > 0 else 0)
        )
            ...
            if "left_right_balance" in rec and rec["left_right_balance"] is not None:
                ...
                try:
                    ...
                    if isinstance(lrb, dict):
                        balance_value: Any = lrb.get("value")
                        if balance_value is not None:
                            stream.left_right_balance[idx] = float(balance_value)
                    else:
                        balance_value = float(lrb)
                        # If raw 0-255 with top-bit flag, mask and re-interpret
                        if balance_value > 100:
                            v_int = int(balance_value)
                            ...
                            stream.left_right_balance[idx] = balance_value
                except (TypeError, ValueError):
                    pass
...
if __name__ == "__main__":  # pragma: no cover
    ...
    quality: np.ndarray = np.full(n, QUALITY_GOOD, dtype=np.uint8)
    elapsed: np.ndarray = np.arange(n, dtype=np.float32)
